...
<script>
  ...
  export default {
    ...
    data() {
      return {
        isProfileChecked: this.PREFERENCES_CHECKBOXES[0].value,
        isNotifsChecked: this.PREFERENCES_CHECKBOXES[1].value,
        isUpdatesChecked: this.PREFERENCES_CHECKBOXES[2].value,
        isNewsChecked: this.PREFERENCES_CHECKBOXES[3].value
      };
    },
    computed: {
      ...mapGetters([
        "PREFERENCES_CHECKBOXES"
      ])
    },
    methods: {
      ...mapActions([
      ...
      clickInput(item) {
        for (let checkbox of this.PREFERENCES_CHECKBOXES) {
